...
const NavItem: FC<INavLink> = ({ path, title, hasSubmenu, subMenu }) => {
    ...
    if (hasSubmenu === "SINGLE") {
        return (
            <li className="relative" onMouseEnter={() => setIsShowing(true)} onMouseLeave={() => setIsShowing(false)}>
                ...
                {isShowing && subMenu && subMenu.length > 0 && <SingleSubMenu links={subMenu as ISubmenu[]} title={title} />}
            </li>
        );
    }
    ...
    if (hasSubmenu === "GROUPED") {
        return (
            <li className="relative" onMouseEnter={() => setIsShowing(true)} onMouseLeave={() => setIsShowing(false)}>
                <NavMenuButton title={title} />
                {isShowing && <GroupedSubMenu title={title} menuGroups={subMenu as ISubmenuGroup[]} />}
            </li>
        );
    }

    return <Navlink href="" title={title} />;
};
